...
def deleteRG() {
    stage("Delete the libcxx resource groups") {
        withEnv(["RESOURCE_GROUP=${XENIAL_RG}"]) {
            oe.azureEnvironment("./cleanup.sh")
        }
        withEnv(["RESOURCE_GROUP=${BIONIC_RG}"]) {
            oe.azureEnvironment("./cleanup.sh")
        }
    }
}
...
try {
    for (int i = 1 ; i <= AGENT_NUM ; i++ ) {
        ACCDeployVM("${XENIAL_LABEL}-${i}".toLowerCase(), "xenial" , "eastus", "${XENIAL_RG}")
        ACCDeployVM("${BIONIC_LABEL}-${i}".toLowerCase(), "bionic", "westeurope", "${BIONIC_RG}")
    }
